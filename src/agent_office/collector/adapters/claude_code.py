...
class ClaudeHookLogAdapter:
    ...
    def apply_command(self, command: ControlCommand) -> AdapterCommandResult:
        ...
        if self.command_outbox_path is None:
            return AdapterCommandResult(False, "claude command outbox is not configured")
        append_command_outbox(self.command_outbox_path, self.runtime_type, command)
        return AdapterCommandResult(True, f"{command.action.value} written to Claude Code command outbox")
